...
import { MOCK_CHART, ZIWEI_PALACES } from '../data/mockData'
      ...
      {selected && (
        <div className="card-glass p-5 animate-fade-up">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setSelected(null)} className="text-ivory/30 hover:text-ivory/60"><ChevronUp size={18} /></button>
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-1">
            {selectedPalace?.stars.map((s, i) => <span key={i} className="bg-navy-700 text-ivory/70 text-xs px-2 py-0.5 rounded-full">{s}</span>)}
            {selectedPalace?.sihua.map((s, i) => <span key={i} className={clsx('text-xs px-2 py-0.5 rounded-full bg-navy-700', SIHUA_COLORS[s])}>{s}</span>)}
          </div>
        </div>
      )}
